(function() {
    ...
    style.textContent = `
        ...
        /* Tela de Carregamento Azul - InfinityCode */
        .eclipse-splash {
            position: fixed;
            top: 0;
            left: 0;
        ...
        .eclipse-splash::after {
            ...
            height: 100%;
            background-image: 
                radial-gradient(2px 2px at 40px 60px, rgba(96, 165, 250, 0.4) 1px, transparent 1px),
                radial-gradient(2px 2px at 80px 120px, rgba(59, 130, 246, 0.3) 1px, transparent 1px),
                radial-gradient(1px 1px at 160px 80px, rgba(37, 99, 235, 0.5) 1px, transparent 1px);
        ...
        /* Botão de Menu Melhorado e Arrastável */
        .eclipse-toggle {
            position: fixed;
            bottom: 30px;
            right: 30px;
        ...
        /* Nova barra de velocidade - tema azul */
    ...
    // Loop de resposta automática - MELHORADO
    (async function autoAnswerLoop() {
        while (true) {
            if (features.autoAnswer) {
                // Função para encontrar e clicar em elementos que contenham "Correto"
                function findAndClickCorrect() {
                    // Procura por elementos que contenham "Correto"
                    const elements = document.querySelectorAll('*');
                    for (const element of elements) {
                        if (element.textContent && element.textContent.includes('Correto')) {
                            // Verifica se é um elemento clicável
                            if (element.tagName === 'BUTTON' || element.tagName === 'DIV' || element.tagName === 'SPAN' || element.tagName === 'LABEL') {
                                element.click();
                                showToast("Clicando em 'Correto' automaticamente", "success", 1000);
                                return true;
                            }
                            // Se não for clicável diretamente, procura o pai clicável
                            let parent = element.parentElement;
                            while (parent) {
                                if (parent.tagName === 'BUTTON' || parent.tagName === 'DIV' || parent.tagName === 'LABEL') {
                                    parent.click();
                                    showToast("Clicando em 'Correto' automaticamente", "success", 1000);
                                    return true;
                                }
                ...
                if (findAndClickCorrect()) {
                        ...
                        try {
                            ...
                            for (const element of elements) {
                                if (element.textContent && (
                                    element.textContent.includes('Verificar') ||
                                    element.textContent.includes('Check') ||
                    ...
                    for (const selector of nextSelectors) {
                        try {
                            ...
                            for (const element of elements) {
                                if (element.textContent && (
                                    element.textContent.includes('Próxima') ||
                                    element.textContent.includes('Next') ||
                                    element.textContent.includes('Continuar')
                                )) {
                                    element.click();
                                    showToast("Indo para próxima questão...", "info", 1000);
                                    break;
                                }
                            }
                        } catch (e) {}
                    ...
                    // Se não encontrou "Correto", tenta clicar na primeira opção
                    ...
                    if (firstOption) {
                        firstOption.click();
                        showToast("Clicando na primeira opção", "info", 1000);
                        await delay(500);
                    }
                }
            }
    ...
    (async function initializeUI() {
        // Cria o splash screen azul com InfinityCode
        const splash = document.createElement("div");
        splash.className = "eclipse-splash";
        splash.innerHTML = `
            <div class="eclipse-splash-content">
                <div class="eclipse-splash-title">InfinityCode</div>
                ...
                <div class="eclipse-splash-status">
                    ...
                    <div>Sistema InfinityCode inicializado</div>
                </div>
            </div>
        `;
        ...
        toggleBtn.onclick = (e) => {
            // Só abre o painel se não estava arrastando
            ...
            <div id="eclipse-tab-about" class="eclipse-tab-content">
                <div class="eclipse-about-content">
                    ...
                    <div class="eclipse-credits">
                        ...
                        InfinityCode • Sempre à frente da curva
                    </div>
                </div>
            </div>
            <div class="eclipse-footer">
                <a href="https://discord.gg/UBRpk7YJ" target="_blank">
                    <span>Comunidade InfinityCode</span>
                </a>
